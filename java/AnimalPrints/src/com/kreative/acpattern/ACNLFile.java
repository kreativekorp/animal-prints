...
public class ACNLFile implements ACBaseFile {
	private byte[] data;
	...
	public PBLCard.PBLEntry[] getPBLPaletteData() {
		...
		for (int i = 0; i < 15; i++) {
			String s1, s2;
			int idx = getPaletteGlobalIndex(i);
			if ((idx & 0xF) == 0xF) {
				s1 = "X";
				s2 = Integer.toString((idx >> 4) + 1);
			} else {
				s1 = Integer.toString((idx >> 4) + 1);
				s2 = Integer.toString((idx & 15) + 1);
			}
	...
	public int[][] getPBLImageData(int frame) {
		...
		for (int dy = ((frame << 9) | 0x6C), y = 0; y < 32; y++, dy += 16) {
